const twoSum = require('./1.two-sum')
const addTwoNumbers = require('./2.add-two-numbers')
const lengthOfLongestSubstring = require('./3.longest-substring-without-repeating-characters')
//console.log(lengthOfLongestSubstring('caas'))
const findMedianSortedArrays = require('./Google/4.median-of-two-sorted-arrays')
//console.log(findMedianSortedArrays([1,3],[2]))
const longestPalindrome = require('./5.longest-palindromic-substring');
const letterCombinations = require('./Google/17.letter-combinations-of-a-phone-number')
const reverse = require('./7.reverse-integer')
//console.log(reverse(-1234))
const maxArea = require('./11.container-with-most-water')
const generateParenthesis = require('./Google/22.generate-parentheses')

//console.log(maxArea([2,3,10,5,7,8,9]))
//console.log(letterCombinations('2456'))
//console.log(findMedianSortedArrays([1,2],[3,4]))
